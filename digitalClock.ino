...
void displayDigitHW() {
  ...
  if (clock_style == 0) {
    for (int pos = 0; pos < 4; pos++) {
      ...
      //     for (int col = 0; col < 5; col++)
      //     {
      //         // tft.fillCircle(oldtmpDigit[row][col][0], oldtmpDigit[row][col][1], r, TFT_BLACK);
      //         if (pos == 0)
      //         {
      //             tft.fillCircle(oldDigit_0[row][col][0], oldDigit_0[row][col][1], r, TFT_BLACK);
      //         }
      //         else if (pos == 1)
      //         {
      //             tft.fillCircle(oldDigit_1[row][col][0], oldDigit_1[row][col][1], r, TFT_BLACK);
      //         }
      //         else if (pos == 2)
      //         {
      //             tft.fillCircle(oldDigit_2[row][col][0], oldDigit_2[row][col][1], r, TFT_BLACK);
      //         }
      //         else if (pos == 3)
      //         {
      //             tft.fillCircle(oldDigit_3[row][col][0], oldDigit_3[row][col][1], r, TFT_BLACK);
      //         }
      //     }
      //     // delay(40);
      // }

      tft.fillRoundRect(x_start - 12, y_start - 12, 5 * isize + 14, 7 * isize + 14, 2, TFT_BLACK);
      ...
      for (int row = startrow; row < 7; row++) {
        for (int col = 0; col < 5; col++) {
          if (pos == 0) {
            oldDigit_0[row][col][0] = tmpDigit[row][col][0];  //= x_start + col * isize - 1 + random(4);
            oldDigit_0[row][col][1] = tmpDigit[row][col][1];  //= y_start + row * isize - 1 + random(4);
          } else if (pos == 1) {
            oldDigit_1[row][col][0] = tmpDigit[row][col][0];  //= x_start + col * isize - 1 + random(4);
            oldDigit_1[row][col][1] = tmpDigit[row][col][1];  //= y_start + row * isize - 1 + random(4);
          } else if (pos == 2) {
            oldDigit_2[row][col][0] = tmpDigit[row][col][0];  //= x_start + col * isize - 1 + random(4);
            oldDigit_2[row][col][1] = tmpDigit[row][col][1];  //= y_start + row * isize - 1 + random(4);
          } else if (pos == 3) {
            oldDigit_3[row][col][0] = tmpDigit[row][col][0];  //= x_start + col * isize - 1 + random(4);
            oldDigit_3[row][col][1] = tmpDigit[row][col][1];  //= y_start + row * isize - 1 + random(4);
          }
          // tft.fillCircle(x_start + col * 7 - 1 + random(4), y_start + row * 7 - 1 + random(4), r, colorrnd);
        }
      }
    ...
    for (int pos = 0; pos < 4; pos++) {
      ...
      tft.fillRoundRect((pos == 0 || pos == 2) ? 0 : 122, (pos < 2) ? 0 : 122, 118, 118, 2, COLOR_MEDIUM[random(12)]);
